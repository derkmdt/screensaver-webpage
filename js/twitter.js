function distanceOfTimeInWords(fromTime, toTime, includeSeconds) {
  ...
  if (distanceInMinutes <= 1) {
    ...
    if (distanceInSeconds < 5) return "5 seconde geleden";
    if (distanceInSeconds < 10) return "10 seconde geleden";
    if (distanceInSeconds < 20) return "20 seconde geleden";
    if (distanceInSeconds < 40) return "30 seconde geleden";
    if (distanceInSeconds < 60) return "een minuut geleden";
    else return "1 minuut geleden";
  }

  if (distanceInMinutes < 45) return distanceInMinutes + " minuten geleden";
  if (distanceInMinutes < 70) return "1 uur geleden";
  if (distanceInMinutes < 1440)
    return Math.round(distanceInMinutes / 60) + " uur geleden";
  if (distanceInMinutes < 2880) return "1 dag geleden";
  if (distanceInMinutes < 43200)
    return Math.round(distanceInMinutes / 1440) + " dagen geleden";
  if (distanceInMinutes < 86400) return "ruim 1 maand geleden";
  if (distanceInMinutes < 525600)
    return Math.round(distanceInMinutes / 43200) + " maanden geleden";
  else return Math.round(distanceInMinutes / 525600) + " jaren geleden";
}
...
function processTweets(data, unique) {
  ...
  $.each(data, function(i, obj) {
    ...
    element.retweetUrl =
      "https://twitter.com/intent/retweet?tweet_id=" + element.tweetId;
    element.replyUrl =
      "https://twitter.com/intent/tweet?in_reply_to=" + element.tweetId;
    element.likeUrl =
      "https://twitter.com/intent/like?tweet_id=" + element.tweetId;
    element.followUrl =
      "https://twitter.com/intent/follow?screen_name=" + element.screenName;
    tweets.push(element);
  });
  return tweets;
}
